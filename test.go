...
func main() {
	...
	recipe.Name = recipe.title(url)
	recipe.Image = recipe.image(url)
	recipe.MemberNum = recipe.people(url)
	resipe.Explanation = recipe.exp(url)

	mat.Quantity = recipe.materialQuantity(url)
	mat.Name = recipe.mat(url)
	resipe.Material = mat
	for i := range recipe.procedure(url) {
